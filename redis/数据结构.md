...
redis作为现今构建互联网应用最常用的中间件之一，我们在使用的同时，肯定也会对其内部的设计和实现感兴趣。redis对使用者暴露的数据类型有string、list、hash、set、sorted set，这篇文章将探究这些数据类型的底层数据结构实现，比如sds、ziplist、quicklist、dict、skiplsit等

本文引用的redis源码版本为redis 6.2

# 一、SDS

redis使用C语言开发，redis确没有直接使用C语言的字符串表示，而是自己构建了一种名叫SDS(simple dynamic string)的数据结构。

这个数据结构跟go语言中的slice切片的底层实现非常相似，下面来详细解析
...
typedef char *sds;
```
sds和char *类型定义成一样，这是为了和传统C语言的字符串保持兼容，但是sds并不等同char *，实际上sds还包含了一个header结构
...
一个完证的sds字符串，由内存地址上前后相邻的两部分组成：一个header，和一个char [] 字节数组

- `len`：字符串的长度

- `alloc`：字符串底层字节数组的容量-1，-1是因为不包含字符串结尾的空字符`\0`

- `buf`：用于保存字符串的字节数组，没有具体长度标识，是一个[柔性数组](https://blog.csdn.net/ce123_zhouwei/article/details/8973073)，柔性数组并不占用结构体的空间
- `flags`：用来标记不同类型的sds，只使用了低3位来标记，高5位暂未使用

根据`alloc`和`len`的类型不同，sds分为几种类型

* sdshdr5：使用5 msb（前5最高有效位）来表示len，  alloc固定是5 msb的最大值 2^5，没有办法动态扩容
...
![img](https://gitee.com/robin_go/images/raw/master/img/sds.png)

SDS_HDR用来从sds字符串获取header的起始指针，而获取header类型的的方法时，从sds字符指针向地址偏移一个字节获取到flag，从flag的低3bit得到header的类型
...
* 内存预分配
  * redis作为数据库，字符串数据经常被频繁修改。C语言的字符串增长或者缩短，就必须对整个底层数组进行一次重新分配，SDS可以在字符串增长时，给SDS分配额外未使用空间，以减少，内存分配次数
  * 字符串小于1M时，翻倍扩容，大于1M时，每次增加1M
* 惰性释放
  * SDS缩短字符串时，只需要通过len字段记录新的字符串长度，而不必立即重新分配内存
  * SDS也提供了相应的API，让我们在需要的时候释放SDS未使用的空间
* 二进制安全
  * SDS使用len字段而不依懒于空字符`\0`来判断字符串的结尾，所以可以使用buf来保存任何二进制格式的数据
  * 比如我们可以使用redis来保存ProtoBuf压缩过的二进制数据
* 兼容C字符串
  * 因为SDS会在字符串的末尾加上`\0`所以能够兼容C字符串，以复用C字符串相关函数

# 二、链表List

redis3.2之前，列表类型的底层实现之一是链表，但在3.2版本之后，列表建就改成quicklist来实现了。不过链表依然在发布订阅、监视器、保存客户端状态信息保存、客户端输出缓冲区等场景被使用，所以做个简单介绍
...
`listNode` 用两个指针 `prev`、`next`保存左右节点，用`void *`指针保存节点的值

`list`保存了头尾指针head、`tail`，方便链表的操作。`len`来记录链表的长度。`dup`、`free`、`match`这三个成员是三个函数的指针，是用于实现多态链表所需的类型特定函数
...
zset和hash对象在元素个数比较少的时候，redis使用压缩列表ziplist来进行存储。
...
![](https://gitee.com/robin_go/images/raw/master/img/redis-ziplist.png)
...
* prevlen 表示前一个节点的长度
  * 当ziplist倒叙遍历时，可以根据prevlen计算出前一个节点的起始地址
...
压缩列表插入和查找的平均复杂度为O(N)，因为ziplist没有指针，所以每次插入或者删除节点，都会发生内存拷贝，所以，ziplist只适合用来保存少量的数据。

# 四、快速列表quicklist

linkedlist 保存每个元素需要 prev 和 next 指针，占据了16个字节，空间附加值太高，而且每个节点都是单独分配，会家具内存的碎片化。因此在 redis3.2 版本之后，使用了新的 quicklist 结构代替 ziplist 和 linkedlist 实现 list 列表类型。

quicklist还是一个双向链表，只不过每个节点都是一个压缩列表

* 压缩列表的长度由参数`list-max-ziplist-size`来控制，用户可以自己设置，默认是8KB
...
>  tip：quicklistNode结构体中count、encoding等字段后面跟着一个冒号和一个数字，这是C语言结构体的位域，它告诉编译器，这个字段所使用的位的宽度，以使内存更加紧凑
...
* recompress：如果我们访问了被压缩的节点数据，需要把数据暂时解压，这是设置recompress=1，标识后面需要吧数据重新压缩

quicklistLZF标识一个被压缩过的ziplist

* sz：压缩后的大小
* compressed：压缩后的数据（柔性数组）

quicklist是quicklist的真正结构

* head、tail 首位指针
* count：元素个数（所有节点元素个数的总和）
* len：节点数
* fill：保存 `list-max-ziplist-sized`的值，表示ziplist的大小
...
字典是一种保存键值对的抽象数据结构，在redis中应用相当广泛，redis的数据库就是使用字典来作为底层实现的，字典还是hash类型的底层实现之一
...
- `size`：是table数组的长度，即哈希表的大小

- `sizemask`：总是等于size-1，一个键的哈希值和sizemask做与运算（&），得到其在数组中的索引位置
...
- `key`：用来保存键值对中的键，void*指针，可以指向任何值
- `v`：用来保存键值对中的值，是一个union结构，可以直接保存uint64_t、int64_t或double类型，也可以是void*

- `next`：指向另一个哈希表节点，组成链表来解决哈希冲突问题，这种通过链表解决哈希冲突的方法通常叫做链地址法（拉链法），速度考虑，新节点总是被添加在链表头的位置（复杂度O(1)）
...
- `rehashidx`：记录了rehash的进度，没有在进行rehash的话，他的值为-1
...
当哈希表保存过多的键值对时，哈希冲突的可能性大大增减，为了保持O(1)的访问复杂度，需要对哈希表进行扩容；哈希表中的键被大量的删除后，缩容能释放哈希表的内存占用。

负载因子（load factor）

负载因子的计算：load_factor = ht[0].uesd / ht[0].size

- 当哈希表的负载因子大于1的时候，redis会对哈希表执行扩容操作（如果服务器正在执行RDB备份，或者AOF文件重写，负载因子大于5才开始扩容）
- 当负载因子小于0.1的时候，会执行收缩操作
...
1. rehashidx记录着rehash的进度，他的值是当前需要被rehash的索引位置，值设置为0，代表rehash开始
2. rehash期间，每次对字典执行的增删改查的同时，也会将ht[0]上rehashidx索引位置的全部键值对rehash到ht[1]上，然后将rehashidx的值加一
3. 除了在对字典操作时会rehash，服务器的定时任务也会主动的进行rehash（可以通过activerehashing参数关闭）
...
大部分情况下跳跃表的效率可以和平衡树媲美，跳跃表实现更简单，范围查询也比平衡树更方便，效率更好
...
- `level`：跳跃表的最大层高，即层数最大的节点的层数（头节点的层数不计算在内）

再来看跳跃表的节点zskiplistNode

- `ele`：保存了一个sds，即zset中的成员，成员不可重复
- `score`：ele对应的分数，跳跃表中，节点按照分数从小到大排序
- `backward`：节点的后退指针，指向前一个节点，从后向前遍历表的时候使用
- `level`：level数组也是柔性数组，具体层数不确定，在新建节点的时候会根据ZSKIPLIST_P来计算出当前节点的层数，越大的层数出现的概率越低
...
- `encoding`：encoding的值决定了contents数组的类型
  - encoding = INTSET_ENC_INT16，contents是一个int16_t类型的数组
  - encoding = INTSET_ENC_INT32，contents是一个int32_t类型的数组
  - encoding = INTSET_ENC_INT64，contents是一个int64_t类型的数组
- `length`：记录了整数集合的元素个数

如果我们将一个新元素插入到整数集合中，并且新元素的类型比现有的所有元素类型都要大，整数集合就要先进行**升级**操作。升级的步骤大致如下：

1. 根据新元素的类型为整数集合底层数组重新分配空间大小（包含新元素的空间）
2. 将底层数组的类型都调整为新的类型，并从后向前将他们放到升级后的位置上
3. 触发升级的新元素要么小于所有元素，要么大于所有元素，所以新元素要么放在数组开头，要么放在末尾

intset通过自动升级底层数组来适应新元素，所以我们可以随意的将int16_t、int32_t、int64_t类型的整数添加到集合中，而不是像普通的数组那样只能保存一种类型的元素；另外又可以确保升级只在需要的时候进行，以此来尽量的节省内存。

intset不支持降级操作。

不像ziplist能够存储任何二进制序列，每个元素采用变长编码，intset只能用来存储整数，每个元素的长度相同（编码相同）
